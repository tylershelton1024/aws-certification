...
- **Flashcards**: every genuine bold term gets exactly one atomic flashcard (1:1 mapping) - no per-section minimum the way questions have. Flashcards are atomic recall aids, not multi-angle testing, so padding out extra near-duplicate cards for the same term doesn't add the same value that multiple question angles do. This extends beyond `**bold**`-marked terms: every acronym (TTL, SSL, CDN, VPC, CIDR, etc.) and every standalone named concept gets its own atomic card too, even if it currently only appears in plain prose or only inside a combined card's definition.
- **Combined "X vs. Y" comparison cards are also valuable and should be kept/added alongside the atomic cards, not instead of them**, whenever two (or more) terms are naturally taught as a contrasting or paired pair - e.g. "TTL vs. Invalidation," "Fault Tolerance vs. High Availability," "Stateful vs. Stateless." The atomic card answers "what is X"; the combined card answers "how do X and Y relate/differ," which is a genuinely different, valuable question a comparison card answers better than two separate atomic cards can. Do not delete an existing combined card just because atomic versions of its terms also exist - both patterns coexist deliberately.
- Periodically audit notes.md/deeper_dive_notes.md's bold terms *and* acronyms against flashcards.md/deeper_dive_flashcards.md for two kinds of gaps: (1) a term with no atomic card at all, (2) a naturally paired/contrasting set of terms with no combined comparison card.
